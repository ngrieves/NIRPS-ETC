...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan  8 17:11:27 2023
...
#wave_matrix = fits.open('june2022_files/NIRPS_WAVE_MATRIX_A.fits') #HA mode from June 2022
#wave_blaze = fits.open('june2022_files/NIRPS_BLAZE_A.fits') #HA mode from June 2022 #wave_blaze = fits.open('NIRPS_S2D_BLAZE_A.fits')
...
for iord in range(len(new_orders)):
    ...
    if wave_range == 'FSR':
        ...
        iord_blazenorm_init = blaze_med_iord[gt50_med] / np.nanmax(blaze_med_iord[gt50_med]) #(blaze_med_iord[gt50_med] - np.nanmin(blaze_med_iord[gt50_med])) / (np.nanmax(blaze_med_iord[gt50_med]) - np.nanmin(blaze_med_iord[gt50_med])) #blaze_med_iord[gt50_med]/max(blaze_med_iord[gt50_med])
        iord_blazenorm_interp = scipy.interpolate.interp1d(np.arange(iord_blazenorm_init.size),iord_blazenorm_init)
        iord_blazenorm = iord_blazenorm_interp(np.linspace(0,iord_blazenorm_init.size-1,order_array_size))
...
wave_range_out.to_csv('updated_files_jan2023/'+new_waverange_filename,index=None,sep='\t')#,format=('%s','%.8f','%.8f','%.8f','%.8f'))
...
#save dataframe
trans_out.to_csv('updated_files_jan2023/'+new_effs_filename,index=None,sep='\t')


###### plots to compare old vs new effs #####
if showplots == 1:
    newlabel = 'Dec2022 (grating2)'
    eff = pd.read_csv('v1_files_simulation/effs.txt',header=0,sep=r"\s+",skiprows=[1])
    eff_june2022 = pd.read_csv('june2022_files/updated_files/NIRPS_effs.txt',header=0,sep=r"\s+",skiprows=[1])
    
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['Order'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['order'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,1],'.',color='blue',alpha=0.7,markersize=2,label=newlabel)
    ...
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['Blaze'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['blaze'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,2],'.',color='blue',alpha=0.7,markersize=2,label=newlabel)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    ...
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['Tel_clean'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['telescope'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_tel[0]*1000.0,new_tel[1],'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,3],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel('Transmission Telescope Clean',fontsize=20)
    plt.legend(fontsize=18)
    plt.tight_layout()
    plt.savefig('plots/transmission_telescope.png')
    
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['Fiber-end'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['frontend'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_frontend[0],new_frontend[4],'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,4],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel('Transmission Frontend',fontsize=20)
    plt.legend(fontsize=18)
    plt.tight_layout()
    plt.savefig('plots/transmission_frontend.png')

    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['Back-End'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['backend'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_backend[0],new_backend[1],'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,5],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel('Transmission Backend',fontsize=20)
    plt.legend(fontsize=18)
    plt.tight_layout()
    plt.savefig('plots/transmission_backend.png')

    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['H4RG'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['H4RG'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_H4RG[0],new_H4RG[1],'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,6],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel('Transmission H4RG',fontsize=20)
    plt.legend(fontsize=18)
    plt.tight_layout()
    ...
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['HA'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['HA_mode'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_fiber_HA[0],new_fiber_HA[2]*new_fiber_HA[7],'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,7],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    #plt.ylim(0.35,0.6)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel(r"Transmission Fiber throughput HA: "+"\n"+" Absorption in fiber T * Total HA measured",fontsize=20)
    ...
    plt.figure(figsize=(12,8))
    plt.plot(eff['#Lambda'],eff['HE'],'.',color='red',alpha=0.7,markersize=1,label='original (simulated)')
    plt.plot(eff_june2022['#wavelength'],eff_june2022['HE_mode'],'.',color='green',alpha=0.7,markersize=1,label='Jun2022 (grating1)')
    plt.plot(new_fiber_HE[0],new_fiber_HE[2]*new_fiber_HE[7]*0.88,'.',color='blue',alpha=0.7,markersize=4,label=newlabel)
    plt.plot((new_transmission_array[:,0]),new_transmission_array[:,8],'.',color='orange',alpha=0.7,markersize=2,label=newlabel+' interpolated')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(900,2000)
    #plt.ylim(0.42,0.65)
    plt.xlabel('Wavelength [nm]',fontsize=20)
    plt.ylabel(r"Transmission Fiber throughput HE: "+"\n"+"Absorption in fiber T * Total HE measured",fontsize=20)
    plt.legend(fontsize=16)
    plt.tight_layout()
    ...
    for i in range(len(HA_Iseeing)):
        plt.plot(eff['#Lambda'],eff[HA_Iseeing[i]],'.',alpha=0.7,label=HA_Iseeing[i]+' original')
    ...
    for i in range(len(HA_Iseeing)):
        plt.plot(eff['#Lambda'],eff[HE_Iseeing[i]],'.',alpha=0.7,label=HE_Iseeing[i]+' original')
...
##########################################################################  
tapas = pd.read_csv('v1_files_simulation/tapas.txt',header=0,sep=r"\s+",skiprows=[1])
ncols_tapas = 8
...
tapas_df.to_csv('updated_files_jan2023/'+new_tapas_filename,index=None,sep='\t')
##########################################################################  
##########################################################################  
...
#NG update 14 July 2022
st_templates = pd.read_csv('v1_files_simulation/STAR_templates.txt',header=0,sep=r"\s+")
st_templates_columns_o = ['f0v', 'f5v', 'g0v', 'g5v', 'k0v', 'k3v', 'k7v', 
                        'm0v','m1v', 'm2v', 'm3v', 'm4v', 'm5v', 'm6v', 'm7v', 'm8v', 
                        'm9v', 'l1v','l2v', 'l3v', 'l5v', 'l6v', 'l8v', 't2v', 'g8v',
                        'b3v','b8v','b9v','a1v'] #add in new spectral types NG 9 June 2022
st_templates_columns_n = ['wavelength','F0V', 'F5V', 'G0V', 'G5V', 'K0V', 'K3V', 'K7V', 
                        'M0V','M1V', 'M2V', 'M3V', 'M4V', 'M5V', 'M6V', 'M7V', 'M8V', 
                        'M9V', 'L1V','L2V', 'L3V', 'L5V', 'L6V', 'L8V', 'T2V', 'G8V',
                        'B3V','B8V','B9V','A1V'] #add in new spectral types NG 9 June 2022
st_templates_new = np.empty([order_array_size*len(new_orders),len(st_templates_columns_n)])
st_templates_new[:,0] = new_transmission_array[:,0]

for icolst in range(len(st_templates_columns_o)):
    st_templates_interp_icol = scipy.interpolate.interp1d(st_templates['#Lambda'],st_templates[st_templates_columns_o[icolst]],fill_value="extrapolate")
    st_templates_new[:,icolst+1] = st_templates_interp_icol(new_transmission_array[:,0])

#st_templates_new_df = pd.DataFrame(data=st_templates_new,index=None,columns=st_templates.columns)
st_templates_new_df = pd.DataFrame(data=st_templates_new,index=None,columns=st_templates_columns_n)
st_templates_new_df.to_csv('updated_files_jan2023/'+new_st_templates_filename,index=None, sep='\t')
